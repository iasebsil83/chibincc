...
str* debugTokens(lst* tokens, ulng depth);
...
str* Value__toStr(valueArg* v, ulng depth) {
	...
	switch(v->id) {
		...
		case VARG__CALL:
			...
			result = str__addChrSelf(result, '"');
			c1 = (call*)(v->content);
			result = str__addSelf(result, c1->name);
			result = str__addChrSelf(result, '"');
			result = str__addChrSelf(result, '(');
			result = str__addChrSelf(result, '\n');
			for(ulng i=0ULL; i < lst__length((lst*)(c1->params)); i++){
				s1 = Value__toStr((valueArg*)lst__index((lst*)(c1->params), i), depth+1ULL);
				result = str__addSelf(result, s1);
				str__free(s1);
				result = str__addChrSelf(result, ',');
				result = str__addChrSelf(result, '\n');
		...
		case VARG__SUBCONTENT:
			...
			result = str__addChrSelf(result, '{');
			result = str__addChrSelf(result, '\n');
			s1 = debugTokens((lst*)(v->content), depth+1ULL);
			result = str__addSelf(result, s1);
			str__free(s1);
			for(ulng i=0; i < depth; i++){ result = str__addChrSelf(result, '\t'); }
			result = str__addChrSelf(result, '}');
